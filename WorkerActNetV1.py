...
class WorkerActNet(nn.Module):
    def __init__(self, input_size=27, conv_channels=1, kernel_size=5, hidden_size=1024, num_classes=16):
        ...
        # 分类任务的输出层
        self.fc_class1 = nn.Linear(hidden_size, num_classes)
        # 分类任务的输出层
#         self.fc_class2 = nn.Linear(hidden_size, num_classes)
        
        # 回归任务的输出层
        self.fc_p1 = nn.Linear(hidden_size, 1)
...
#         norm_cat = self.max_pool(norm_cat)
        
        # LSTM 处理
#         print(norm_cat.shape, "====")
        ...
        # 分类任务
        class1_out = self.fc_class1(lstm_out)
        # class_out.shape: (batch_size, num_classes)
        
        # 回归任务
        p1_out = self.fc_p1(lstm_out)
        # offset_out.shape: (batch_size, 1)
        
                # 分类任务
#         class2_out = self.fc_class2(lstm_out)
        # class_out.shape: (batch_size, num_classes)
        
        # 回归任务
#         p2_out = self.fc_p2(lstm_out)
        # offset_out.shape: (batch_size, 1)
        
#         return class1_out, p1_out, class2_out, p2_out
